...
document.addEventListener("DOMContentLoaded", () => {
            ...
            return {
                ...
                layers: f.layers.map(l => {
                    return {
                        ...
                        // img (HTMLImageElement) rimane con lo stesso riferimento poiché immutabile
                    };
                })
            };
        });
    ...
    function decodeUploadedReferenceGif(arrayBuffer, layer) {
        try {
            ...
            // Propaga le proprietà decodificate a tutti i frame del riferimento globale
            propagateLayerChanges(layer, {
                gifFrames: layer.gifFrames,
                isAnimatedGif: true,
                w: layer.w,
                h: layer.h,
    ...
    function handleTabBtnClick(e) {
        ...
        const header = tab.closest(".window-tabs-header");
    ...
    function initTabDockingSystem() {
        ...
        document.addEventListener("drop", (e) => {
            // Se stiamo droppando sopra una testata di tab, non fare nulla (è gestito da handleHeaderDrop)
            ...
            createNewFloatingWindow(x, y, tabBtn, tabContent);
    ...
    function updateTabHeadersDropzones() {
        ...
        document.querySelectorAll(".tab-btn").forEach(tab => {
            ...
            const currentWin = tab.closest(".window");
            if (currentWin && !tab.hasAttribute("data-origin-win")) {
                tab.setAttribute("data-origin-win", currentWin.id);
            }
    ...
    function handleHeaderDragLeave(e) {
        e.currentTarget.classList.remove("drag-over-tabs");
    }
    ...
    function handleHeaderDrop(e) {
        ...
        if (!targetWin || sourceWin === targetWin) return;
    ...
    function handleTabDragEnd(e) {
        ...
        document.querySelectorAll(".window-tabs-header").forEach(h => {
            h.classList.remove("drag-over-tabs");
        });
    }
    ...
    function saveLayoutToLocalStorage() {
        ...
        localStorage.setItem("gifstudio_layout_v5", JSON.stringify(layoutData));
        localStorage.setItem("gifstudio_layout_v5_screen", JSON.stringify({ w: window.innerWidth, h: window.innerHeight }));
    }
    ...
    function loadLayoutFromLocalStorage() {
        ...
        try {
            ...
            // Se il layout salvato è incompleto o obsoleto (manca qualcuna delle 5 finestre),
            // viene ripristinato il layout predefinito.
            const requiredIds = ["win-project", "win-tools", "win-properties", "win-canvas", "win-timeline"];
            const hasAll = requiredIds.every(id => layoutData && layoutData.hasOwnProperty(id));
            if (!hasAll) {
                arrangeWindowsDefault();
                return;
            }
            ...
            // Leggi le dimensioni dello schermo salvate
            const savedScreenStr = localStorage.getItem("gifstudio_layout_v5_screen");
            let savedW = screenW;
            let savedH = screenH;
            if (savedScreenStr) {
                try {
                    const savedScreen = JSON.parse(savedScreenStr);
                    savedW = savedScreen.w || screenW;
                    savedH = savedScreen.h || screenH;
                } catch(err) {}
            }

            const sizeChanged = (savedW !== screenW || savedH !== screenH);
            ...
            for (const [id, val] of Object.entries(layoutData)) {
                ...
                if (win) {
                    ...
                    // Ripristino dello stato "Sopra Tutto" (Pinned)
            ...
            // Forza sempre l'adattamento al viewport all'avvio per prevenire posizioni errate!
            adaptLayoutToScreenSize(savedW, savedH, screenW, screenH, true);
        } catch (e) {
    ...
    function applyGlobalFilters(layer, sourceImg) {
        ...
        const bgRemoveHash = layer.bgRemoveActive ? `bg_${layer.bgRemoveColor}_${layer.bgRemoveTolerance}_${layer.bgRemoveSeedX}_${layer.bgRemoveSeedY}` : "bg_none";
        const cacheKey = `${layer.id}_filters_${replacementsHash}_${bgRemoveHash}`;
    ...
    function initFilterTools() {
        ...
        dom.btnAddReplacement.addEventListener("click", () => {
            state.colorReplacements.push({
                ...
                transparent: false,
                seedX: state.lastPickedReplaceCoords ? state.lastPickedReplaceCoords.x : null,
                seedY: state.lastPickedReplaceCoords ? state.lastPickedReplaceCoords.y : null
            });
            state.lastPickedReplaceCoords = null; // reset
            updateReplacementsUI();
            filterCache.clear();
            requestRender();
        });
    ...
    function initUiCustomizer() {
        ...
        dom.uiColorBg.addEventListener("input", (e) => {
            updateThemeColors();
            document.querySelectorAll(".theme-btn").forEach(btn => btn.classList.remove("active"));
        });
        dom.uiColorWin.addEventListener("input", (e) => {
            updateThemeColors();
            document.querySelectorAll(".theme-btn").forEach(btn => btn.classList.remove("active"));
        });
        dom.uiColorText.addEventListener("input", (e) => {
            updateThemeColors();
            document.querySelectorAll(".theme-btn").forEach(btn => btn.classList.remove("active"));
        });
        dom.uiColorAccent.addEventListener("input", (e) => {
            updateThemeColors();
            document.querySelectorAll(".theme-btn").forEach(btn => btn.classList.remove("active"));
        });
    ...
    function bindGlobalInputs() {
        ...
        // Ascolta resize del browser per ricalcolare il layout dinamicamente con debounce
        let resizeTimeout;
        window.addEventListener("resize", () => {
            ...
            resizeTimeout = setTimeout(() => {
                ...
                adaptLayoutToScreenSize(savedW, savedH, screenW, screenH);
    ...
    function saveCustomNames() {
        ...
        document.querySelectorAll(".window").forEach(win => {
            ...
            if (titleEl && titleEl.innerText) names[win.id] = titleEl.innerText;
        });
        document.querySelectorAll(".tab-btn").forEach(tab => {
            const tabId = tab.getAttribute("data-tab");
            if (tabId && tab.innerText) names[tabId] = tab.innerText;
        });
        localStorage.setItem("gifstudio_custom_names_v1", JSON.stringify(names));
    }
    ...
    function startApp() {
        ...
        // Carica layout salvato o esegui disposizione automatica adattiva
        loadLayoutFromLocalStorage();
        loadCustomNames();
        if (typeof window.updateDynamicUI === "function") window.updateDynamicUI();

        // Esegui controlli e riposizionamento ritardati per gestire la massimizzazione asincrona di Electron
        setTimeout(() => {
            const screenW = window.innerWidth;
            const screenH = window.innerHeight;
            const savedScreenStr = localStorage.getItem("gifstudio_layout_v5_screen");
        ...
        // Inizializza con un messaggio grafico di benvenuto amichevole
        // Rimosso livello di benvenuto
        buildTimelineUI();
        updateReplacementsUI();
