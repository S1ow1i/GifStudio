    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                string targetDir = Path.Combine(appDataDir, "GifStudioPortable", "v1.0.0");
                string mainExe = Path.Combine(targetDir, "Gif Studio.exe");

                // Estrai i file solo se la directory non esiste ancora (ottimizzazione tempo avvio)
                if (!Directory.Exists(targetDir))
                {
                    using (FileStream fs = new FileStream(exePath, FileMode.Open, FileAccess.Read, FileShare.Read))
                    {
                        // Cerca la firma del file ZIP (PK\x03\x04) usando un buffer in memoria per prestazioni istantanee
                        byte[] sig = new byte[] { 0x50, 0x4B, 0x03, 0x04 };
                        byte[] buffer = new byte[65536]; // Buffer da 64KB in RAM
                        int bytesRead;
                        long currentPos = 0;
                        long zipOffset = -1;
                        ...
                        while ((bytesRead = fs.Read(buffer, 0, buffer.Length)) > 0)
                        {
                            ...
                            // Gestisci il caso in cui la firma si trovi a cavallo del limite del buffer
                        ...
                        // Leggi il payload ZIP in memoria per consentire un indirizzamento relativo corretto al ZipArchive
                ...
                // Avvia l'applicazione desktop principale ed attendi la chiusura
